#!/usr/bin/env bash
#
# SetupFX — restore Postgres (and optionally uploads + timescaledb) from
...
for i in $(seq 1 30); do
  if docker compose -f docker-compose.yml -f docker-compose.prod.yml exec -T postgres \
       pg_isready -U "${POSTGRES_USER:-setupfx}" >/dev/null 2>&1; then
    break
  fi
  sleep 1
done
...
gunzip -c "$TMP_PG" | docker compose -f docker-compose.yml -f docker-compose.prod.yml \
  exec -T postgres psql -U "${POSTGRES_USER:-setupfx}" -d postgres -v ON_ERROR_STOP=1
...
if [[ -n "$TS_DUMP" ]]; then
  ...
  for i in $(seq 1 30); do
    if docker compose -f docker-compose.yml -f docker-compose.prod.yml exec -T timescaledb \
         pg_isready -U "${TIMESCALE_USER:-setupfx}" >/dev/null 2>&1; then
      break
    fi
    sleep 1
  done
  ...
  gunzip -c "$TMP_TS" | docker compose -f docker-compose.yml -f docker-compose.prod.yml \
    exec -T timescaledb psql -U "${TIMESCALE_USER:-setupfx}" -d postgres -v ON_ERROR_STOP=1
fi
