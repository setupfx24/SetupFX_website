    ...
    <section className="bg-white">
      <div className="w-full mx-auto px-6 md:px-10 lg:px-16 pb-10 md:pb-12">
        ...
        >
          {cols.map((col) => (
            <div key={col.id} className="flex flex-col gap-3">
              <h3 className="text-xs font-bold uppercase tracking-[0.18em] text-[#1074FE]">
                {col.heading}
              </h3>
              <ul className="flex flex-col gap-2">
                {col.links.map((link) => (
                  <li key={link}>
                    <a
                      href="#"
                      className="text-sm text-gray-600 hover:text-[#1074FE] transition-colors"
                    >
                      {link}
                    </a>
                  </li>
        ...
        <div className="mt-12 max-w-3xl mx-auto flex flex-col sm:flex-row items-center justify-center gap-6 text-sm text-gray-700">
          ...
          >
            <Phone className="w-4 h-4 text-[#1074FE]" strokeWidth={2} />
            +33 7 59 15 99 87
          </a>
          <span className="hidden sm:inline w-px h-4 bg-gray-300" aria-hidden="true" />
          <span className="inline-flex items-center gap-2 text-center">
            <MapPin className="w-4 h-4 text-[#1074FE] shrink-0" strokeWidth={2} />
