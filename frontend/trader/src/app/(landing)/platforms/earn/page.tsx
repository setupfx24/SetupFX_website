  ...
  return (
    <main className="relative overflow-hidden" style={{ background: 'var(--fx-bg)' }}>
      ...
      <section className="fx-container relative z-10 pt-28 md:pt-36 pb-16">
        <p className="text-xs uppercase tracking-[0.25em] text-accent/85 mb-3">Earn</p>
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <Link href="/earn/tasks" className="inline-flex items-center gap-2 px-5 py-3 rounded-lg bg-accent text-bg-base font-bold text-sm hover:brightness-110">
            Start Earning <ArrowRight size={14} />
          </Link>
          <Link href="/earn/store" className="inline-flex items-center gap-2 px-5 py-3 rounded-lg border border-accent/40 text-text-primary text-sm hover:border-accent/70">
            Explore Rewards
          </Link>
        </div>
      </section>
      ...
      <section className="fx-container relative z-10 py-20">
        <div className="rounded-2xl border border-accent/30 p-10 md:p-14 text-center bg-[rgba(16,116,254,0.04)]">
          ...
          <div className="flex justify-center gap-3 flex-wrap">
            ...
            <Link href="/earn/tasks" className="inline-flex items-center gap-2 px-6 py-3 rounded-lg border border-accent/40 text-text-primary text-sm hover:border-accent/70">
              Go to Dashboard
            </Link>
          </div>
        </div>
...
function Card({ icon: Icon, title, tagline, body }: { icon: any; title: string; tagline: string; body: string }) {
  return (
    <div className="rounded-xl border border-accent/25 p-6 bg-[rgba(255,255,255,0.02)]">
      ...
      <p className="text-xs uppercase tracking-wider text-accent/80 mb-3">{tagline}</p>
...
function MiniCard({ icon: Icon, title, body }: { icon: any; title: string; body: string }) {
  return (
    <div className="rounded-lg border border-accent/20 p-5 bg-[rgba(255,255,255,0.02)]">
