        ...
        {COLUMNS.map((col, i) => {
          ...
          return (
            ...
            >
              ...
              >
                {/* Use semantic `text-foreground` so the heading reads
                    correctly under both the dark home theme (cream) and
                    the light sub-landing theme (slate). No hardcoded
                    white/black. */}
                <span className="font-display uppercase text-sm sm:text-base tracking-tight text-foreground">
                  {col.heading}
                </span>
                <ChevronDown
                  className={`size-4 shrink-0 transition-transform ${isOpen ? 'rotate-180' : ''} ${isOpen ? 'text-primary' : 'text-foreground/85'}`}
                  aria-hidden
                />
              </button>
