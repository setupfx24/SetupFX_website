...
export const CanvasRevealEffect: React.FC<CanvasRevealEffectProps> = ({
  ...
  useEffect(() => {
    ...
    // Caller passes a non-empty `colors` array (defaults to a 6-tone
    // palette upstream), so `colors[i]` is safe within these bounded
    // ranges — non-null assertions keep the array typed as `number[][]`
    // for the `.map` consumer below.
    if (colors.length === 0) return; // defensive: shouldn't happen, but bail rather than crash
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        u_colors: {
          /* colorsArray entries are [r, g, b] tuples — indices 0-2 are
           * always present by construction (see colors normalization
           * earlier in this effect). */
