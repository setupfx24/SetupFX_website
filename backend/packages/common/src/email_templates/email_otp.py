...
def render_email_otp(
    *,
    ...
    subject = f"SetupFX verification code: {code}"
    html = render_layout(
        title="Verify your email",
        intro=f"Hi {name}, here's your one-time code to confirm this email on SetupFX.",
        body_html=body,
        footer_note=(
            "This code is valid for one verification only. Do not share it "
            "with anyone — SetupFX staff will never ask you for it."
        ),
    )
    ...
    text_lines = [
        f"Hi {name},",
        "",
        "Here's your SetupFX email verification code:",
        "",
        f"   {code}",
        "",
