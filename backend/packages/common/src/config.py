...
class Settings(BaseSettings):
    ...
    DATABASE_URL: str = "postgresql+asyncpg://setupfx:setupfx_dev@localhost:5432/setupfx"
    TIMESCALE_URL: str = "postgresql+asyncpg://setupfx:setupfx_dev@localhost:5433/marketdata"
    ...
    # Cookie Domain attribute. Set to a parent domain (e.g. ".setupfx24.com") to share
    ...
    ADMIN_EMAIL: str = "admin@setupfx24.com"
    ...
    # HMAC credentials — must match SETUPFX_API_KEY / SETUPFX_API_SECRET in the Corecen .env.
    ...
    # a position, SetupFX pushes the trade to Corecen's broker API for LP routing.
    # These credentials are the API key/secret registered in Corecen's admin panel
    # for the SetupFX broker account.
    ...
    # BSC testnet RPC for the SetupFXVaultV1 testnet deploy. Falls back
...
def _assert_production_secrets(s: Settings) -> None:
    ...
    if s.ENVIRONMENT.lower() != "production":
        ...
        import logging
        log = logging.getLogger("setupfx.config")
        weak_jwt = [
    ...
    if bad:
        raise RuntimeError(
            ...
            "set them in /opt/setupfx/.env before deploying."
        )
